...
void matchmake_users()
{
    ...
    for (int i = 0; i < client_count; i++)
    {
        if (clients[i].group_id == -1)
        { // Pas encore assigné à un groupe
...
void *group_chat(void *arg)
{
    ...
    {
        buffer[nbytes] = '\0'; // Assurer que le buffer est une chaîne de caractères valide

        pthread_mutex_lock(&clients_mutex);

        // Rechercher le groupe auquel appartient le client
        int group_id = -1;
        char sender_name[50]; // Assuming the name length is less than 50 characters
        for (int i = 0; i < client_count; i++)
        {
            if (clients[i].socket == sockfd)
            {
                ...
                sender_name[sizeof(sender_name) - 1] = '\0'; // Ensure null-termination
                break;
            }
        }
        ...
        if (group_id != -1)
        {
            char message_with_name[BUFFER_SIZE + 51];                                     // Buffer to hold the message with the sender's name
            int max_message_length = sizeof(message_with_name) - strlen(sender_name) - 3; // 3 for ": " and null terminator

            if (strlen(buffer) > max_message_length)
            {
                buffer[max_message_length] = '\0'; // Truncate the buffer if necessary
            }

            snprintf(message_with_name, sizeof(message_with_name), "%s: %s", sender_name, buffer);

            for (int i = 0; i < client_count; i++)
            {
                if (clients[i].group_id == group_id && clients[i].socket != sockfd)
                {
                    if (send(clients[i].socket, message_with_name, strlen(message_with_name), 0) < 0)
                    {
    ...
    // Supprimer le client de la liste lorsqu'il se déconnecte
    close(sockfd);
    pthread_mutex_lock(&clients_mutex);
    for (int i = 0; i < client_count; i++)
    {
...
void *client_handler(void *arg)
{
    ...
    // Recevoir les données du formulaire : nom, sexe et âge
    ...
    sscanf(buffer, "%s %s %d", user.name, user.gender, &user.age);
